...
class FinancialCreditExtractor:
    ...
    def financial_credit_sms_extraction(self):
        try:
            ...
            for item in self.sms_data:
                ...
                if channel in ['apollo', '8500']:
                
                    financial_keywords = ["debited", "disbursed", "paid", "credited"]
                    ...
                    if found:
                        try:
                            ...
                            response = model.generate_content(f"In this given transaction SMS text, {body}, please extract important information like account, transaction type, amount, transaction reason, balance, transaction date, in json format of key:value")
                            response.resolve()
                            ...
                            sms_info_dict = {
                                "sms_channel":channel,
                                "account": sms_cleaned_json.get("account"),
                                "transaction_type": sms_cleaned_json.get("transaction_type"),
                                "amount": sms_cleaned_json.get("amount"),
                                "transaction_reason": sms_cleaned_json.get("transaction_reason"),
                                "balance": sms_cleaned_json.get("balance"),
                                "date": date
                            }
...
if __name__ == "__main__":
    ...
    extract_sms = FinancialCreditExtractor(sms_data)
    extracted_transport_expense = extract_sms.financial_credit_sms_extraction()
